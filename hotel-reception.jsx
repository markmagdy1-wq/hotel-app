...
function HotelReceptionApp({ role, username, onLogout }) {
  ...
  const addUser = (username, password, userRole) => {
    const candidate = username.trim().toLowerCase();
    const candidateHash = sha256Hex(candidate);
    const taken =
      candidateHash === AUTH_SYSTEM_USERNAME_HASH ||
      candidateHash === AUTH_ANALYTICS_USERNAME_HASH ||
      extraUsers.some((u) => u.username.toLowerCase() === candidate);
    ...
    return (
      ...
      >
        ...
        <main style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
          ...
          {managerTab === "users" && (
            <UsersTab extraUsers={extraUsers} onAddUser={addUser} onDeleteUser={deleteUser} />
          )}
          {managerTab === "pricing" && (
            <RoomPricingTab
              roomRates={roomRates}
...
function UsersTab({ extraUsers, onAddUser, onDeleteUser }) {
  ...
  const submit = () => {
    setFormError("");
    setFormNote("");
    const result = onAddUser(form.username, form.password, form.role);
    if (!result.ok) {
      setFormError(result.error);
      return;
    }
    const accessLabel = form.role === "analyst" ? "view-only analytics access" : "reception access";
    setFormNote(`"${form.username.trim()}" can now sign in with ${accessLabel}.`);
  ...
  return (
    <div>
      ...
      {extraUsers.length === 0 ? (
        <div style={{ color: TOKENS.inkSoft, fontSize: "0.9rem" }}>No additional users yet — the main reception login still works.</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {extraUsers.map((u) => (
            <div key={u.id} style={{ ...cardStyle, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <div style={{ fontSize: "0.78rem", color: TOKENS.inkSoft }}>
                  {u.role === "analyst" ? "Analytics access (view only)" : "Reception access"}
                </div>
              </div>
              <button onClick={() => onDeleteUser(u.id)} style={ghostBtn}>
                Remove
              </button>
            </div>
          ))}
